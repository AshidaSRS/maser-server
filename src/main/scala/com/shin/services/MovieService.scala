...
trait MovieService[F[_]] {
  ...
  val repo: MovieRepository[F]

  val model: String = classOf[Movie].getSimpleName

  def insert(item: Movie): F[Option[Movie]] =
    for {
  ...
  def retrieve(id: Long): F[Option[Movie]] =
    for {
  ...
  def update(item: Movie): F[Option[Movie]] =
    for {
  ...
  def batchedInsert(items: List[Movie]): F[List[Option[Movie]]] =
    for {
      _ <- L.debug(s"Trying to insert batch $model")
      insertedItems <- items.traverse(repo.insert)
    } yield insertedItems

  def batchedUpdate(items: List[Movie]): F[List[Option[Movie]]] =
    for {
