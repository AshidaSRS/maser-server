    ...
    private def parseIterable[U](source: Input, in: Input, p: Parser[U], res: ParseResult[U]): ParseResult[U] = {
      res match {
        case s@Success(v: Option[_], n) if v.isEmpty =>
          if (!n.atEnd) parseIterable(source, in.rest, p, p(in.rest)) else s.copy(next = source).asInstanceOf[AnyElemNameParser.this.ParseResult[U]]
        case s@Success(v: Iterable[_], n) if v.isEmpty =>
          if (!n.atEnd) parseIterable(source, in.rest, p, p(in.rest)) else s.copy(next = source).asInstanceOf[AnyElemNameParser.this.ParseResult[U]]
        case s@Success(v, n) =>
          s.copy(next = n.erased(in.pos.column - 1, n.pos.column - 1)).asInstanceOf[AnyElemNameParser.this.ParseResult[U]]
        case other =>
          other
      }
    }
  ...
  def reads(seq: scala.xml.NodeSeq, stack: List[ElemName]): Either[String, A] = seq match {
    case node: scala.xml.Node =>
      parse(parser(node, stack), node.child) match {
        case x: Success[_] => Right(x.get.asInstanceOf[A])
